"""
Unmodified sampling.py from FedFA: https://ieeexplore.ieee.org/abstract/document/10286887
"""
...
def trainset_sampling_label(
    args, trainset, sample_rate, rare_class_nums, noniid_labeldir_part
):
        ...
        for k in range(args.num_classes):
            ...
            if len_class != 0:
                ...
                if k in d:
                    ...
                    dict_class[k] = list(
                        idxs_labels[0, index_curent_class:index_next_class]
                    )
...
def trainset_sampling_label_uniform(
    args, trainset, sample_number, rare_class_nums, noniid_labeldir_part
):
    dict_users_train = {i: [] for i in range(args.K)}
    for i in range(args.K):
...
def trainset_sampling_mixed_digit(
    args, clients_index, trainset, sample_rate, rare_class_nums, labeldir_part
):
...
def trainset_sampling_label_femnist(
    args, trainset, sample_rate, rare_class_nums, noniid_labeldir_part
):
    dict_users_train = {i: [] for i in range(args.K)}
    for i in range(args.K):
