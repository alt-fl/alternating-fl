...
def test_on_globaldataset(args, global_model, dataset_test):
    ...
    for batch_idx, (imgs, labels) in enumerate(Dte):
        ...
        with torch.no_grad():
            ...
            features, log_probs = global_model(
                imgs
            )  # , restricting_classifier = args.RC, train= False
...
def verify_feature_consistency(
    args, client_model, dataset, dict_user_verify, numpy=True
):
    client_model.eval()
    # testing
...
def globalmodel_test_on_localdataset(args, global_model, dataset_test, dict_user_test):
    ...
    for i in range(args.K):
        ...
        correct = 0.0
        Dte = DataLoader(
            DatasetSplit(dataset_test, dict_user_test[i]),
            batch_size=args.TB,
            shuffle=True,
        )  #
...
def globalmodel_test_on_specifdataset(
    args, client_index, global_model, dataset_test, dict_user_test
):
    ...
    correct = 0.0
    Dte = DataLoader(
        DatasetSplit(dataset_test, dict_user_test[client_index]),
        batch_size=args.TB,
        shuffle=True,
    )
