...
def run_FedFA():
    ...
    results_path = f"results/mnist/plain-fedfa/client_{int(args.C * args.K)}_{args.K}/"
    ...
    trainset = torchvision.datasets.MNIST(
        root=root, train=True, download=True, transform=trans_mnist
    )
    testset = torchvision.datasets.MNIST(
        root=root, train=False, download=True, transform=trans_mnist
    )
    ...
    for i in range(args.K):
        ...
        for k in label_class:
            training_number[i][k] = list(
                trainset.targets[list(dict_users_train[i])]
            ).count(k)
    ...
    if Train_model:
        ...
        if similarity:
            similarity_dictfa = torch.load(
                "results/Test/label skew/fmnist/fedfa/seed{}/similarity_dictfa_{}E_{}class.pt".format(
                    args.seed, args.E, C
                )
            )
        acc_listfa = torch.load(
            "results/Test/label skew/fmnist/fedfa/seed{}/acc_listfa_{}E_{}class.pt".format(
                args.seed, args.E, C
            )
        )
        global_modelfa = server_feature.nn
        client_modelsfa = server_feature.nns
        path_fedfa = "results/Test/label skew/fmnist/fedfa/seed{}/global_model_fedfa_{}E_{}class".format(
            args.seed, args.E, C
        )
        global_modelfa.load_state_dict(torch.load(path_fedfa))
        for i in range(args.K):
            path_fedfa = "results/Test/label skew/fmnist/fedfa/seed{}/client{}_model_fedfa_{}E_{}class".format(
    ...
    if save_models:
        ...
        #     torch.save(
        #         similarity_dictfa,
        #         "results/Test/label skew/fmnist/iid-fedavg/seed{}/similarity_dictfa_{}E_{}class.pt".format(
        #             args.seed, args.E, C
        #         ),
        #     )
        # torch.save(
