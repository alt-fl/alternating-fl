...
def run_FedFA():
    ...
    results_path = f"results/{args.dataset.lower()}/fhe{str(args.ratio).replace('.', '_')}_inter{args.AR}_{args.SR}/"
    ...
    trainset_unnorm = CIFAR100(
        root=root, train=True, download=True, transform=ToTensor()
    )
    train_loader = DataLoader(trainset_unnorm, batch_size=64)
    means, stds = get_mean_std(train_loader)
    print(f"mean: {means.numpy()}\nstd: {stds.numpy()}")
    del trainset_unnorm
    del train_loader

    specf_model = model.ClientModel(args, name="cifar100").to(args.device)
    ...
    trainset = CIFAR100(root=root, train=True, download=True, transform=trans_cifar100)
    testset = CIFAR100(root=root, train=False, download=True, transform=trans_cifar100)
    ...
    # perform partition
    noniid_labeldir_part = CIFAR100Partitioner(
        auth_dst.targets,
    ...
    csv_file = (
        "data/CIFAR100/cifar100_noniid_labeldir_clients_"
    ...
    syn_noniid_labeldir_part = CIFAR100Partitioner(
        synth_dst.targets,
        num_clients=num_clients,
        balance=None,
        partition="shards",
    ...
    if Train_model:
        ...
        if similarity:
            similarity_dictfa = torch.load(
                "results/Test/label skew/cifar100/fedfa/seed{}/similarity_dictfa_{}E_{}class.pt".format(
                    args.seed, args.E, C
                )
            )
        acc_listfa = torch.load(
            "results/Test/label skew/cifar100/fedfa/seed{}/acc_listfa_{}E_{}class.pt".format(
                args.seed, args.E, C
            )
        )
        global_modelfa = server_feature.nn
        client_modelsfa = server_feature.nns
        path_fedfa = "results/Test/label skew/cifar100/fedfa/seed{}/global_model_fedfa_{}E_{}class".format(
            args.seed, args.E, C
        )
        global_modelfa.load_state_dict(torch.load(path_fedfa))
        for i in range(args.K):
            path_fedfa = "results/Test/label skew/cifar100/fedfa/seed{}/client{}_model_fedfa_{}E_{}class".format(
