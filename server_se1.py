...
class Server:
    ...
    def fedavg_joint_update(
        ...
        for t in range(self.args.r):
            ...
            # # local updating
            self.nns, self.loss_dict = client_update(
                self.args,
                index,
                self.nns,
                self.nn,
            ...
            if similarity:
                ...
                if fedbn:
                    ...
                    for testset_index, testset_per in enumerate(testset):
                        ...
                        mean_feature_similarity0 = compute_mean_feature_similarity(
                            self.args,
                            index,
                            self.nns,
                            self.dataset,
                            self.dict_users,
                            testset_per,
                            dict_users_test[datasets_name[testset_index]],
                        )
                        mean_feature_similarity_list.append(mean_feature_similarity0)
                    mean_feature_similarity = torch.mean(
                        torch.Tensor(mean_feature_similarity_list)
                    )
                else:
        ...
        if iid:
            for k in range(self.args.K):
                path = "results/Test/{} skew/{}/iid-fedavg/seed{}/client{}_model_fedavg_{}E_{}class".format(
                    self.args.skew,
                    self.args.dataset,
                    self.args.seed,
                    k,
            ...
            for k in range(self.args.K):
                path = "results/Test/{} skew/{}/fedavg/seed{}/client{}_model_fedavg_{}E_{}class".format(
                    self.args.skew,
                    self.args.dataset,
                    self.args.seed,
                    k,
    ...
    def fedprox_joint_update(
        ...
        for t in range(self.args.r):
            ...
            # # proximal updating
            self.nns, self.loss_dict = client_prox_update(
                self.args,
                index,
                self.nns,
                self.nn,
        ...
        mean_CKA_dict = acc_list
        for k in range(self.args.K):
            path = "results/Test/{} skew/{}/fedprox/seed{}/client{}_model_fedprox_{}E_{}class".format(
                self.args.skew,
                self.args.dataset,
                self.args.seed,
                k,
    ...
    def feddyn(
        ...
        for t in range(self.args.r):
            ...
            self.nns, self.loss_dict = client_feddyn(
                self.args,
                index,
                preround_nns,
                self.nns,
        ...
        mean_CKA_dict = acc_list
        for k in range(self.args.K):
            path = "results/Test/{} skew/{}/feddyn/seed{}/client{}_model_feddyn_{}E_{}class".format(
                self.args.skew,
                self.args.dataset,
                self.args.seed,
                k,
    ...
    def moon(
        ...
        for t in range(self.args.r):
            ...
            # joint updating to obtain personalzied model based on updating global model
            self.nns, self.loss_dict = client_moon(
                self.args,
                index,
                preround_nns,
                self.nns,
        ...
        mean_CKA_dict = acc_list
        for k in range(self.args.K):
            path = "results/Test/{} skew/{}/moon/seed{}/client{}_model_moon_{}E_{}class".format(
                self.args.skew,
                self.args.dataset,
                self.args.seed,
                k,
    ...
    def fedproc(
        ...
        for t in range(self.args.r):
            ...
            # joint updating to obtain personalzied model based on updating global model
            self.cls, self.nns, self.loss_dict = client_fedproc(
                self.args,
                index,
                self.contrals,
                self.nns,
        ...
        for k in range(self.args.K):
            path = "results/Test/{} skew/{}/fedproc/seed{}/client{}_model_fedproc_{}E_{}class".format(
                self.args.skew,
                self.args.dataset,
                self.args.seed,
                k,
    ...
    def fedfa_anchorloss(
        ...
        for k in range(self.args.K):
            path = "results/Test/{} skew/{}/fedfa/seed{}/client{}_model_fedfa_{}E_{}class".format(
                self.args.skew,
                self.args.dataset,
                self.args.seed,
                k,
    ...
    def fedbn(
        ...
        testsets,
        dict_test,
        clients_dataset_index,
        similarity=False,
        fe_optimizer_name="fedavg",
        test_global_model_accuracy=False,
    ):
        ...
        for t in range(self.args.r):
            ...
            if fe_optimizer_name == "fedavg":
                for k in index:
                    ...
                    self.nns, loss_dict = client_update(
                        self.args,
                        k_set,
                        self.nns,
                        self.nn,
                        t,
                        self.dataset[client_dataset_index],
                        self.dict_users,
                        loss_dict,
                    )
                ...
                for k in index:
                    ...
                    k_set = [k]
                    loss_dict = copy.deepcopy(self.loss_dict)
                    self.nns, loss_dict = client_prox_update(
                        self.args,
                        k_set,
                        self.nns,
                        self.nn,
                ...
                for k in index:
                    ...
                    k_set = [k]
                    loss_dict = copy.deepcopy(self.loss_dict)
                    self.nns, loss_dict = client_feddyn(
                        self.args,
                        k_set,
                        preround_nns,
                        self.nns,
                ...
                for k in index:
                    ...
                    k_set = [k]
                    loss_dict = copy.deepcopy(self.loss_dict)
                    self.nns, loss_dict = client_moon(
                        self.args,
                        k_set,
                        preround_nns,
                        self.nns,
                ...
                for k in index:
                    ...
                    k_set = [k]
                    loss_dict = copy.deepcopy(self.loss_dict)
                    self.cls, self.nns, loss_dict = client_fedproc(
                        self.args,
                        k_set,
                        self.contrals,
                        self.nns,
                ...
                for k in index:
                    ...
                    k_set = [k]
                    loss_dict = copy.deepcopy(self.loss_dict)
                    self.cls, self.nns, loss_dict = client_fedfa_cl(
                        self.args,
                        k_set,
                        self.cls,
                        self.nns,
